...
class ShowLargestFoldersFilesOnlyController {
    ...
    public function execute(Database $database) {
        ...
        //
        $rows = $this->fetchAssoc($database, sprintf("SELECT * FROM fileentries WHERE type in ('dir') ORDER BY dir_size DESC LIMIT %s OFFSET %s", $pagination->getItemsPerPage(), $pagination->calcOffset()));
        $table = [];
        foreach ($rows as $row) {
        ...
        (new Table(
            ...
            ['DUI-table__col--grow', 'DUI-table__col--number', 'DUI-table__col--number', 'DUI-table__col--number', 'DUI-table__col--number']
        ))
            ->withPercentBar(4, 4)
            ->withData($table)
